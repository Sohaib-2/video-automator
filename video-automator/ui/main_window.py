...
class MainWindow(QMainWindow):
    ...
    def check_system_requirements(self):
        ...
        if gpu_available:
            print("[GPU] NVIDIA GPU detected - will use hardware acceleration")
        else:
            print("[INFO] No NVIDIA GPU detected - will use CPU encoding")
    ...
    def open_enhanced_settings(self, sample_folder):
        ...
        if dialog.exec_() == QDialog.Accepted:
            self.settings = dialog.get_settings()
            self.save_settings()
            self.status_label.setText("[OK] Settings configured!")
    ...
    def scan_and_add_folders(self, parent_folder: str):
        ...
        self.status_label.setText(f"[SCAN] Scanning {parent_folder}...")
        ...
        if found_folders:
            ...
            if skipped_folders:
                summary += f"[WARN] Skipped {len(skipped_folders)} folder(s):\n"
            ...
            self.status_label.setText(f"[OK] Added {len(found_folders)} videos from scan")
        else:
            QMessageBox.warning(
                self,
            ...
            self.status_label.setText("[ERROR] No valid projects found")
    ...
    def add_folder_to_queue(self, folder_path, silent=False):
        ...
        if not silent:
            self.status_label.setText(
                f"[OK] Added: {folder_name} ({num_images} image(s)) • "
    ...
    def clear_queue(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            self.video_queue = []
            self.start_btn.setEnabled(False)
            self.status_label.setText("[OK] Queue cleared")
    ...
    def start_rendering(self):
        ...
        self.status_label.setText(
            f"[START] Starting batch render: {len(self.video_queue)} videos "
        ...
        for video in self.video_queue:
            if video['path'] == folder_path:
                if success:
                    ...
                    self.status_label.setText(f"[ERROR] Failed: {video['name']}")
                break
    ...
    def on_all_complete(self):
        ...
        self.status_label.setText(
            f"[DONE] All {len(self.video_queue)} video(s) complete! "
    ...
    def dropEvent(self, event):
        ...
        if len(folders) == 1:
            ...
            if reply == QMessageBox.Yes:
                ...
                if skipped:
                    summary += f"[WARN] Skipped {len(skipped)} folder(s):\n"
                ...
                self.status_label.setText(f"[OK] Added {added} videos via drag & drop")
                ...
                self.status_label.setText(
                    f"[OK] Added {total_added} videos from {len(folders)} parent folder(s)"
                )
